...
r <- reactions_1_2_3_4 %>% 
  ...
  arrange(log_H4SiO4) %>% 
  plot_ly() %>% 
  add_trace(x = ~log_H4SiO4, y = ~log_K_H_ratio, mode = "lines+markers", marker = list(color = "black")) %>%
...
subplot(p, r, nrows = 2) %>% animation_opts(500, transition = 100, redraw = TRUE)
```



Cumulative animation option
```{r}
