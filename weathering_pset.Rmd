...
#coefficients of change for weathering of k spar to gibbsite (reaction 2)
co_spar_2 <- -1
co_H_2 <- -1
co_H2O_2 <- 0.5
co_gibbs_2 <- -2
...
kspar_to_kaol <- kspar_to_kaol %>% filter(mol_l_K <= 1.01e-4)
...
# Reaction 4: K feldspar weathering to kaolinite
...
kspar_to_musc <- kspar_to_musc %>% filter(steps <= 100)

reactions_1_2_3_4 <- union(reactions_1_2_3, kspar_to_musc)
```

Plot evolution of water chemistry during k feldspar weathering to muscovite
```{r}
